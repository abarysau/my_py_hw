...
print('PIZZERIA sample program v4\n')
...
class Pizza:
    ...
    def GET_menu_from_json(self, json):
        # HW for lecture 14
        self.__pizza_list = []
        for i in range(len(json)):
            self.__pizza_list.append((json[i].get('pizza_name'), json[i].get('pizza_size')))
        print('The menu was reloaded from JSON server (pizza14.json)')   
...
# Part 0: Work with PIZZA menu via JSON server
input_str = input('\nWould you like to load menu from JSON server (pizza14.json)? (Y/N):')

while (input_str.upper() == 'Y'):
    try:
        reply = requests.get("http://localhost:3000/menu")
    except requests.RequestException:
        print('Communication error')
    else:
        if reply.status_code == requests.codes.ok:
            my_pizza.GET_menu_from_json(reply.json())
        else:
            print('Server error')        
